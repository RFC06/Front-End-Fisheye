...
function getMediaByPhotographerId(id) {
  return data.medias.filter(media => media.photographerId === id);
}
        ...
        if (item.image) {
            ...
            mediaCard.innerHTML = `
                <video controls height="200px">
                    <source src="assets/media/${photographer.name}/${item.video}" type="video/mp4">
...
function initPhotographerPage() {
  ...
  if (photographerId) {
      ...
      console.error('ID de photographe non valide.');
  }
}
